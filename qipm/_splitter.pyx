...
from sklearn.utils._typedefs cimport float32_t, float64_t, intp_t, int8_t, int32_t, uint32_t, uint64_t
...
cdef inline intp_t rand_pdf(uint32_t* random_state, intp_t low, 
    ...
    cdef:
        float64_t cumulative_sum = 0.0
        float64_t last_sum = 0.0
        float64_t random_value
        intp_t i = 0
        intp_t first_nonzero_prob = 0
        intp_t K = high - low
    ...
    if K < 1:
        ...
        with gil: 
            raise ValueError(
                "The index values contains inconsistent values"
            )
    ...
    for i in range(K):
        ...
        if last_sum == 0 and cumulative_sum > 0:
            # The first nonzero probability is useful if the 
            # random_value is zero.
            first_nonzero_prob = i
        last_sum = cumulative_sum
...
cdef inline int node_split_best(
    ...
    const int8_t[:] monotonic_cst,
    const float64_t[:] feature_bias # const float64_t* feature_bias = NULL
) except -1 nogil:
    """Find the best split on node samples[start:end]
...
cdef inline float32_t median3(float32_t* feature_values, intp_t n) noexcept nogil:
    ...
    cdef float32_t a = feature_values[0], b = feature_values[<intp_t>(n / 2)], c = feature_values[n - 1]
    if a < b:
        if b < c:
            return b
        elif a < c:
...
cdef inline int node_split_random(
    ...
    while (f_i > n_total_constants and  # Stop early if remaining features
        ...
        if feature_bias is None: # if feature_bias == NULL:
            # Draw a feature at random uniform distribution
            f_j = rand_int(n_drawn_constants, f_i - n_found_constants, random_state)
        else:
            # Draw a feature at random following a given probability distribution function (PDF)
...
cdef inline void binary_search(const int32_t[::1] sorted_array,
    ...
    index[0] = -1
    while start < end:
        pivot = <intp_t>(start + (end - start) / 2)
...
cdef class BestSplitter(Splitter):
    ...
    cdef int init(
        ...
        Splitter.init(self, X, y, feature_bias, sample_weight, missing_values_in_feature_mask)
    ...
    cdef int node_split(
        ...
        return node_split_best(
            ...
            self.monotonic_cst,
            self.feature_bias # &self.feature_bias[0],
        )
