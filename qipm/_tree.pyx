...
cdef class TreeBuilder:
    ...
    cpdef build(
        ...
        object X,
        const float64_t[:, ::1] y,
        const float64_t[:] feature_bias=None,
...
cdef class DepthFirstTreeBuilder(TreeBuilder):
    ...
    cpdef build(
        ...
        splitter.init(X, y, feature_bias, sample_weight, missing_values_in_feature_mask)
...
cdef class BestFirstTreeBuilder(TreeBuilder):
    ...
    cpdef build(
        ...
        # tree._qipm_oob(X, y, sample_weight, 3)
...
cdef inline cnp.ndarray float64t_ptr_to_ndarray(float64_t* data, intp_t size):
    """Return copied data as 1D numpy array of intp's."""
    cdef cnp.npy_intp shape[1]
    shape[0] = <cnp.npy_intp> size
    return cnp.PyArray_SimpleNewFromData(1, shape, cnp.NPY_FLOAT64, data).copy()
